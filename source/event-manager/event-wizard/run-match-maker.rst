...
Click the "Execute Match Maker" button to generate the randomized alliances.
...
* Team - Team Number
* Matches - number of matches played which are used to calculate rankings. This number should match the number in "Matches Per Team" at the top of the Build Schedule screen.
* Surrogates - number of matches played in which this team is a surrogate. Teams should only play 1 surrogate match maximum in most cases.
...
* For example: "7796(2)" (**green box**) means the team on that row will be on an alliance with team 7796 twice over the course of all Qualification matches.
...
Select the file using the "Browse..."" button, and click Import Matches from File. The imported alliance pairings overwrite the alliances generated by the software, but retain the scheduled match times. Do not use the "Execute Match Maker" button if a custom pairing is being loaded.
